# -*- coding: utf-8 -*-
"""
Created on Mon Aug  5 10:54:02 2019
...
def save_to_excel(file_name, list_of_df=[], list_of_datatypes=[]):
    ...
    writer = pd.ExcelWriter(file_name+'.xlsx', engine='xlsxwriter') # Create a Pandas Excel writer using XlsxWriter
    i=0
    for df in list_of_df:
        df.to_excel(writer, sheet_name=list_of_datatypes[i]) # Convert the dataframe to an XlsxWriter Excel object
        i+=1
...
def display_team_stats(WR_df, Played_df, team_wr, max_player=5, team_name=''):
    ## Displays a bar plot of heroes WR and number played for first 5/6 player in a team
#    ax=[None]*6
